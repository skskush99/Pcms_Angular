...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule , CommonModule , NgSelectModule , ButtonComponent],
...
export class AddCourtComponent {
  ...
  divisionDropdown : DropdownListInterface[] = [];
    ...
    ngOnInit(): void {
      ...
      console.log(this.editCourt);
      this.getDivisionDropdown();
      console.log(this.editCourt?.DivisionId);
      ...
      if(this.editCourt){
        ...
        if(this.addCourtForm.value.division)this.getDistrictDropdown('init');
        console.log(this.addCourtForm.value);
        
        
      }
      
      
      // this.addCourtForm.get('date')?.valueChanges.subscribe(val => {
    ...
    onSave(){
      ...
      let reqParam = {
        ...
        data : {
          ...
          "jCourtHindi": this.addCourtForm.value.courtEng || '',
