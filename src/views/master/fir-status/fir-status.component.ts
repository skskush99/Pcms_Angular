      ...
      next: (res: any) => {
        if (res.status && res.data?.length) {
          ...
          const columnHeaders: { [key: string]: string } = {
            RowID:                   'Sr No',
            FirStatusNameEnglish:    'FIR Status English Name',
            FirStatusNameHindi:      'FIR Status Hindi Name',
            IsActive:                'Status'
          };
          ...
          const modifiedData = res.data.map((row: any, index: number) => {
            const modifiedRow: { [key: string]: any } = {
              'Sr No': index + 1,
              'FIR Status English Name': row.FirStatusNameEnglish || '',
              'FIR Status Hindi Name': row.FirStatusNameHindi || '',
      ...
      next: (res: any) => {
        if (res.status && res.data?.length) {
          ...
          const exportData = res.data.map((row: any, index: number) => ({
            RowID: index + 1,
            FirStatusNameEnglish: row.FirStatusNameEnglish || '',
            FirStatusNameHindi: row.FirStatusNameHindi || '',
          ...
          const columns = [
            ...
            { header: 'FIR Status English Name',    dataKey: 'FirStatusNameEnglish' },
            { header: 'FIR Status Hindi Name',      dataKey: 'FirStatusNameHindi' },
            { header: 'Status',                     dataKey: 'IsActive' }
          ];

          // ✅ Export to PDF
          this.excel.exportAllJsonPDF(
            headers,
            columns,
            exportData,
            'FIR Status List',
