...
export class PoliceStationComponent {
  ...
  getPoliceCircleDropdown() {
    this.api.get(this.url.getPoliceCircleDropdown()).subscribe({
      next:  (res: any) => { this.policeCircleDropdown = res.data; },
      error: () => {}
    });
  }
  ...
  exportPDF() {
    ...
    const columns = [
      ...
      { header: 'Police Station', dataKey: 'StationNameEng' },
      { header: 'Police Circle',  dataKey: 'CircleNameEng' }
    ];

    this.excel.exportAllJsonPDF(headers, columns, this.policeStationList, 'Police Station List', true);
  }
}